...
union() {
    ...
    if ($include_wiring_harness)
        wiring_harness( num_conductor = 4, harness_length = 20, connector_type = "socket", $fn=40);
    
    if ($include_back)  neopixel_stick_case_back(screw_case = screw_case, flush_perim = flush_case);
    if ($include_front) neopixel_stick_case_front(screw_case = screw_case,
